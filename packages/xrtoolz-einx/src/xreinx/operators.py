"""Layer-1 ``Operator`` wrappers around :mod:`xreinx` functions.

One operator per Layer-0 function (D7). Each subclasses
:class:`xrcore.Operator`, so it inherits DataTree leaf-wise dispatch and
...
from xreinx._src._pattern import infer_output_signature
...
class Einsum(Operator):
    ...
    Applies :func:`xreinx.einsum` using DataArray dim names as the
...
class Rearrange(Operator):
    ...
    Applies :func:`xreinx.rearrange`, e.g. ``"(h w) c -> h w c"`` to
...
class Reduce(Operator):
    ...
    Applies :func:`xreinx.reduce`, e.g. ``"b [i] j -> b j"`` to reduce
...
class Repeat(Operator):
    ...
    Applies :func:`xreinx.repeat`, e.g. ``"h w -> h w c"`` with
...
class Matmul(_BinaryLinalgOp):
    ...
    Applies :func:`xreinx.matmul`: sums the product over ``dim``,
...
class Outer(_BinaryLinalgOp):
    ...
    Applies :func:`xreinx.outer`: the result carries every dim of the
...
class BatchMatmul(_BinaryLinalgOp):
    ...
    Applies :func:`xreinx.batch_matmul`, contracting ``dim`` while
