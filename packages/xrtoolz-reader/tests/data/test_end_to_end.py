...
def test_cmems_open_passes_credentials_when_absent(tmp_path, monkeypatch):
    # No explicit credentials, no env vars, no ~/.cmems file in a sandboxed HOME.
    # load_cmems() also walks CWD's parents for a ``.env``, so chdir into the
    # sandbox: the workspace root ships one and it would otherwise be found.
