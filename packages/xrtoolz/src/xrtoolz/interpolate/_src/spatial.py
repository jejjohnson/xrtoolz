...
def _build_tree(src_xy: Float[np.ndarray, "n two"], metric: Metric) -> Any:
...
def _to_metric_xy(
    lons: Float[np.ndarray, "n"], lats: Float[np.ndarray, "n"], metric: Metric
) -> Float[np.ndarray, "n two"]:
...
def _bandwidth_rule(pts: Float[np.ndarray, "n d"], rule: BandwidthRule) -> float:
