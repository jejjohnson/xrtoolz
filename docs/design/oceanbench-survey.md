...
| A.1.6 | [`temporal.py`](https://github.com/jejjohnson/oceanbench/blob/neurips-paper/oceanbench/_src/geoprocessing/temporal.py) | `time_rescale` (`(t-t0)/dt` → float seconds with pint units). Useful for spectral analysis where `time` needs to be numeric. | Skip — already shipped at [`transforms/_src/encoders/coord_time.py:14`](src/xrtoolz/transforms/_src/encoders/coord_time.py#L14) with `Operator` wrapper |
