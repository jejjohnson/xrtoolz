...
## Type Contract — Two Tiers (xarray, Operator)
...
|---|---|---|---|---|
| **Layer 0 — xarray** | `xr_toolz.<module>` (private `_src/`) | `xr.DataArray` (single-variable) or `xr.Dataset` + variable selectors (multi-variable) | `xr.DataArray` or `xr.Dataset` | `dim=` |
| **Layer 1 — Operator** | `xr_toolz.<module>` | `xr.Dataset` (or two for multi-input) | `xr.Dataset` \| `xr.DataArray` \| scalar (terminal viz returns `matplotlib.Figure / Axes`, see D10) | constructor `variable=` / `dims=` |

Rules:

- Each tier delegates downward; logic is never duplicated. Layer 1 wraps Layer 0.
- Layer 0 uses arity to disambiguate the input type: single-variable functions take `xr.DataArray`; multi-variable functions take `xr.Dataset` plus explicit variable selectors (`variable=`, `u_var=`, …).
- Layer 1 input is always `xr.Dataset` (or two `xr.Dataset` for multi-input operators). Output is **usually** `xr.Dataset` for transformations that preserve the dataset shape, but reduction-style operators (e.g., metrics) may return an `xr.DataArray` or scalar, and terminal viz operators return `matplotlib.Figure / Axes` (D10). Composition (`Sequential`, `Graph`) only sees Layer 1.
- Numpy / scipy kernels still exist as private siblings in `_src/_<name>_kernels.py` where the Layer 0 wrapper needs a function to hand to `xr.apply_ufunc`. They are an implementation detail — not a public surface and not part of the type contract.

Example — `metrics.rmse`:

```python
# Layer 0 — xarray (Dataset in, DataArray out)
xr_toolz.metrics.rmse(pred_ds, ref_ds, variable="ssh", dims=["time"])
