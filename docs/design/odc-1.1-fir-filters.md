...
|---|---|---|
| Butterworth IIR low/high/band/stop | [`_smooth_kernels.lowpass_filter`](../../src/xr_toolz/interpolate/_src/_smooth_kernels.py) | Unchanged |
...
```python
# src/xr_toolz/interpolate/_src/_smooth_kernels.py
def _fir_taps(
    *,
...
| Slice | LOC | Notes |
|---|---|---|
| `_fir_taps` + `fir_filter` (private numpy kernel) | 50 | scipy does the heavy lifting |
