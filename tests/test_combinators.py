"""Tests for the xrtoolz-specific combinators ``Augment`` and ``ApplyToEach``.

Pipekit's `Tap` is exercised by pipekit's own test suite; this module
covers only the xarray-Dataset-flavoured combinators that ship in
`xrtoolz.combinators`.
"""
...
from xrtoolz import ApplyToEach, Augment, Operator, Sequential
